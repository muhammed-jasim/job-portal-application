...
def job_creation(request):
    categories = Jobs.CATEGORY_TYPE  # Get categories from the model
    job_type = Jobs.JOB_TYPES
    ...
    if request.method == 'POST':
        ...
        data = {
            ...
            'categories': request.POST['job-category'],  # Corrected key
        ...
        context = {"job_created": True, "title": new_job.title, "categories": categories,'job_types':job_type, "job_location":location}
        return render(request, 'admin/job_creation_form.html', context)

    return render(request, 'admin/job_creation_form.html', {'categories': categories,'job_types':job_type,"job_location":location})
...
def job_listing(request):
    jobs = Jobs.objects.all()  # Start with all jobs
    count = jobs.count()
    categories = Jobs.CATEGORY_TYPE  # Get the categories for the dropdown
    job_types = Jobs.JOB_TYPES  # Get the job types for the dropdown
    ...
    if selected_category and selected_category != 'all_categories':
        query &= Q(categories=selected_category)  

    if selected_job_type and selected_job_type != 'all_type':
        query &= Q(job_type=selected_job_type)  
    ...
    if experience:
        if experience == '1-2':
            query &= Q(experience=1) | Q(experience=2)
        elif experience == '2-3':     
            query &= Q(experience=2) | Q(experience=3)
        elif experience == '3-6':
            query &= Q(experience__gte=3, experience__lte=6)
        elif experience == '6':
            query &= Q(experience__gte=6)
    ...
    context = {
        ...
        'selected_jobValue' : selected_category,
        'selected_jobtype' : selected_job_type,
